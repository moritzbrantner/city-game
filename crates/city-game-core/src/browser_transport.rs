...
use crate::{
    CityCommand, CityQuery, CitySave, CityScenario, build_save_render_frame,
};
...
pub(crate) fn new_save_json(scenario_json: &str) -> String {
    encode_result((|| {
        let scenario: CityScenario = serde_json::from_str(scenario_json)
...
pub(crate) fn execute_json(save_json: &str, command_json: &str) -> String {
    encode_result((|| {
        let mut save: CitySave = serde_json::from_str(save_json)
            .map_err(|error| error.to_string())?;
        let command: CityCommand = serde_json::from_str(command_json)
            .map_err(|error| error.to_string())?;
...
pub(crate) fn query_json(save_json: &str, query_json: &str) -> String {
    encode_result((|| {
        let save: CitySave = serde_json::from_str(save_json)
            .map_err(|error| error.to_string())?;
        let query: CityQuery = serde_json::from_str(query_json)
            .map_err(|error| error.to_string())?;
...
pub(crate) fn render_frame_json(save_json: &str, aspect: f32) -> String {
    encode_result((|| {
        let save: CitySave = serde_json::from_str(save_json)
            .map_err(|error| error.to_string())?;
        let frame = build_save_render_frame(&save, aspect)
            .map_err(|error| error.to_string())?;
...
mod tests {
    use crate::{
        CityCommand, CityQuery, CitySave, CityScenario, ExternalRevision,
    ...
    fn browser_transport_executes_the_same_command_contract() {
        ...
        let command_json = serde_json::to_string(&CityCommand::AdvanceFixedSteps { steps: 4 }).unwrap();

        let response: Value = serde_json::from_str(&execute_json(&save_json, &command_json)).unwrap();
    ...
    fn browser_transport_queries_the_same_read_contract() {
        ...
        let response: Value = serde_json::from_str(&query_json(&save_json, &query_json)).unwrap();
