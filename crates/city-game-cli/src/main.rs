...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.as_slice() {
        ...
        [_, command, input, output] if command == "frame" => frame(input, output),
        _ => {
            eprintln!("usage:");
            eprintln!("  city-game-cli import <input.osm.pbf> <scenario.json>");
            eprintln!("  city-game-cli frame <scenario.json> <frame.json>");
            std::process::exit(2);
        }
    }
}
...
fn frame(input: &str, output: &str) -> Result<(), Box<dyn Error>> {
    let scenario: CityScenario = serde_json::from_slice(&fs::read(input)?)?;
    let frame = build_render_frame(&scenario, 16.0 / 9.0)?;
